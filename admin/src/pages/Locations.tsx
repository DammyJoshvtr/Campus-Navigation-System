...
import { getLocations, deleteLocation, createLocation, updateLocation, uploadImage } from '../services/api';
...
const Locations = () => {
  ...
  const [formData, setFormData] = useState({ name: '', type: '', latitude: '', longitude: '', description: '', image: '' });
  ...
  const handleOpenModal = (loc = null) => {
    if (loc) {
      ...
      setFormData({ name: '', type: '', latitude: '', longitude: '', description: '', image: '' });
    }
    setIsModalOpen(true);
  };
    ...
    <div className="animate-in fade-in duration-500">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Add, edit, or remove campus locations.</p>
        </div>
        <button
            ...
            <tbody className="divide-y divide-slate-100">
              {locations.length === 0 ? (
                ...
                <tr key={loc.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-slate-600">{loc.type || 'N/A'}</td>
                  <td className="px-6 py-4 text-slate-600">
                    <div className="text-xs">Lat: {loc.coordinate?.latitude || loc.latitude}</div>
                    <div className="text-xs">Lng: {loc.coordinate?.longitude || loc.longitude}</div>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      loc.approval_status === 'approved' ? 'bg-emerald-100 text-emerald-800' :
                      loc.approval_status === 'rejected' ? 'bg-red-100 text-red-800' :
