...
import { getEvents, deleteEvent, createEvent, updateEvent, uploadImage } from '../services/api';
import { Plus, Pencil, Trash2, Loader2, Image as ImageIcon, CalendarDays } from 'lucide-react';
...
const Events = () => {
  ...
  const [formData, setFormData] = useState({ title: '', description: '', locationName: '', date: '', time: '', status: 'upcoming', image: '', author: '' });
  ...
  const handleOpenModal = (evt = null) => {
    if (evt) {
      ...
      setFormData({ title: '', description: '', locationName: '', date: '', time: '', status: 'upcoming', image: '', author: '' });
    }
    setIsModalOpen(true);
  };
    ...
    <div className="animate-in fade-in duration-500">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Create and manage campus events.</p>
        </div>
        <button
            ...
            <tbody className="divide-y divide-slate-100">
              {events.length === 0 ? (
                ...
                <tr key={evt.id} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center">
                      <div className="h-10 w-10 flex-shrink-0 bg-slate-100 rounded-lg overflow-hidden flex items-center justify-center mr-4 border border-slate-200">
                        {evt.image ? (
                          <img src={evt.image} alt="" className="h-full w-full object-cover" />
                        ) : (
                      ...
                      <div>
                        ...
                        <div className="text-xs text-slate-500">By {evt.author || 'Unknown'}</div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 text-slate-600">{evt.locationName}</td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      evt.approval_status === 'approved' ? 'bg-emerald-100 text-emerald-800' :
                      evt.approval_status === 'rejected' ? 'bg-red-100 text-red-800' :
                ...
                <div className="md:col-span-2">
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="flex-1">
                      ...
                      />
                      {uploading && <span className="text-xs text-primary-600 mt-2 block">Uploading...</span>}
                    </div>
                  </div>
                  {formData.image && (
                    <input 
                      ...
                      className="w-full mt-2 px-3 py-2 border border-slate-300 rounded-xl text-sm" 
